		    ...
		    if ($db) {
		    	if (isset($_POST['submit'])){
		    		if (isset($_POST['search'])){
		    			$search = $_POST['search'];
		    			$sql = pg_query($db, "SELECT * from login WHERE email != 'admin@gmail.com' AND (email ilike '%$search%' OR username ilike '$search')");
			    		$row = pg_fetch_array($sql);
				        if ($row == NULL){
				        	echo '<div style="font-weight: bold; font-size: 25px; text-align: center;">No User found!!!</div>';
				        }
				        else{
				        	$sql1 = pg_query($db, "SELECT * from login WHERE email != 'admin@gmail.com' AND (email ilike '%$search%' OR username ilike '$search')");
					        $i = 1;
					       	?>
					        ...
					        <?php
					   		while($row1 = pg_fetch_array($sql1)){
					   			...
					   			<th scope="row"><?php echo $i; ?></th>
						   			...
						   			<td class="avatar"><img src="./profile/<?php echo $row1['profile']; ?>"></td>
					   			<?php
					   			$i++;
					   		}
				        }
		    		...
		    		$sql = pg_query($db, "SELECT * from login WHERE email != 'admin@gmail.com'");
			        $i = 1;
			        ?>
			        <table class="table table-striped">
					  <thead>
			        ...
			        <?php
			   		while($row = pg_fetch_array($sql)){
			   			...
			   			<th scope="row"><?php echo $i; ?></th>
				   			...
				   			<td class="avatar"><img src="./profile/<?php echo $row['profile']; ?>"></td>
			   			<?php
			   			$i++;
			   		}
		    	}
